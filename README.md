# マルハン綾瀬上土棚店・設定狙い分析アプリ

Streamlit + Supabase で動く、ログイン付きの ana-slo 日別ページ分析アプリです。  
管理者が `マルハン綾瀬上土棚店` の日別ページURLまたは保存HTMLを1件ずつ登録し、友達はログイン後に分析結果だけ閲覧できます。
...
requestsで取得できない、またはPlaywright環境が用意できない場合でも、ブラウザで保存したHTMLファイルを管理画面からアップロードして解析できます。
...
- Playwright fallbackは環境にChromiumが入っていないと動きません。その場合は保存HTML取込を使えます。
