...
def render(df, calendar_df, profile):
    ...
    with tab_day:
        fig = px.bar(daily, x="date", y="avg_diff", color="avg_diff", color_continuous_scale=["#2563eb", "#f8fafc", "#dc2626"], title="日別平均差枚", labels=layout.COLUMN_LABELS)
        st.plotly_chart(fig, use_container_width=True)
        st.dataframe(layout.style_diff_columns(daily, ["total_diff", "avg_diff"]), use_container_width=True, hide_index=True)
    with tab_weekday:
        fig = px.bar(weekday, x="weekday", y="avg_diff", color="win_rate", title="曜日別傾向", labels=layout.COLUMN_LABELS)
        st.plotly_chart(fig, use_container_width=True)
